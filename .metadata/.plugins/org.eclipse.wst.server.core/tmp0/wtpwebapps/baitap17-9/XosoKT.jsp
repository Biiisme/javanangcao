...
<body>
	...
	<table class="table table-bordered" style="margin-bottom: 0px">
		 <tr>
		 	<th style="text-align: center;font-size: 25px; color: white; background-color: red;">
   		 Xổ số kiến thiết ngày: <%= request.getParameter("ngay") %>/<%= request.getParameter("thang") %>/<%= request.getParameter("nam") %>
			</th>
		</tr>
	</table>
	<table class="table table-bordered select" style="margin-bottom: 0px">
		...
		<tr>
  			<td > 
	 			<form action="XosoKT.jsp" method="get"> 
				<select id="selectNgay" style="width: 100px;position: relative; left: 200px" name="ngay">
					<option value="<%= request.getParameter("ngay") %>"><%= request.getParameter("ngay") %>	
					</option>
					<%for(int i = 1 ; i <= 31 ; i ++){ %>
			...
			<td>
				<select id="selectThang" style="width: 100px ;position: relative; left: 200px" name="thang">
					<option value="<%= request.getParameter("thang") %>"><%= request.getParameter("thang") %>	
					</option>
					<%for(int i = 1 ; i <= 12 ; i ++){ %>
					<option value="<%=i%>"><%=i %></option>
					<% 
			...
			<td >
				<select  id="selectNam" style="width: 100px ; position: relative; left: 200px" name="nam">
					<option value= "<%= request.getParameter("nam") %>"><%= request.getParameter("nam") %>	
					</option>
